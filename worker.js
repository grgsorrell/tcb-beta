...
export default {
  async fetch(request, env, ctx) {
    ...
    // punchlist2 item 1: server-enforced lookup fall-through helpers.
    // buildLookupFallthroughQuery constructs the grounding query for an empty
    // lookup from the tool's own arguments + fact class (federal → FEC, state →
    // official state source). lookupResultLacksData detects a no-verified-data
    // tool_result (status 'unsupported'/stub, no usable data) while NOT
    // re-searching when the lookup endpoint already attached its own
    // web_search_fallback excerpt.
    function buildLookupFallthroughQuery(name, input) {
      const inp = input || {};
      const stateFull = expandStateName(inp.state || '') || (inp.state || '');
      const office = inp.office || '';
      const yr = inp.race_year || new Date().getFullYear();
      const juris = inp.jurisdiction_name || inp.district || '';
      const federal = isFederalOffice(office);
      switch (name) {
        case 'lookup_finance_reports':
          return federal
            ? `FEC ${yr} quarterly reporting deadlines candidate committee`
            : `${stateFull} campaign finance report deadlines ${office} ${yr} site official`;
        case 'lookup_compliance_deadlines':
          return `${stateFull} ${office} ${yr} filing qualifying deadline secretary of state`;
        case 'lookup_donation_limits':
          return federal
            ? `FEC ${yr} individual contribution limits candidate committee`
            : `${stateFull} ${office} contribution limits ${yr}`;
        case 'lookup_jurisdiction':
          return `${stateFull} ${juris} municipalities parishes communities ${yr}`;
        default:
          return `${stateFull} ${office} ${yr}`.trim();
      }
    }
    function lookupResultLacksData(content) {
      let s = '';
      if (typeof content === 'string') s = content;
      else if (Array.isArray(content)) s = content.map(b => (b && typeof b.text === 'string') ? b.text : (typeof b === 'string' ? b : JSON.stringify(b))).join(' ');
      else if (content && typeof content === 'object') s = JSON.stringify(content);
      const lc = s.toLowerCase();
      if (!lc.trim()) return false;
      // The endpoint already ran its own web_search_fallback and attached an
      // excerpt — data is present, don't double-search.
      if (lc.includes('web_search_fallback') || lc.includes('"web_search"') || lc.includes('excerpt')) return false;
      // No-verified-data signals from the lookup endpoints.
...
`;
      ...
      // punchlist2 item 2: put the state's local-subdivision term IN ground truth
      // (proximity beats a distant rule). Louisiana has parishes, Alaska has
      // boroughs; everywhere else "county" is correct and needs no note.
      const SUBDIVISION_TERMS = { LA: { sing: 'parish', plur: 'parishes' }, AK: { sing: 'borough', plur: 'boroughs' } };
      const _sub = SUBDIVISION_TERMS[(state || '').toUpperCase().trim()];
      const subdivisionLine = _sub
...
RETURNING USER: Greet warmly, reference their campaign naturally, jump right into helping. Don't re-explain who you are.`;
      ...
      if (_questionCategory === 'factual' || _questionCategory === 'compliance' || _questionCategory === 'predictive') {
        systemPrompt += '\n\n================================================================\nGROUNDING MANDATE FOR THIS TURN\n================================================================\n' +
          'GROUNDING MANDATE — this ' + _questionCategory + ' turn requires Search Grounding for any specific date, dollar amount, named contact, current event, compliance rule, or biographical claim. Do NOT use training-data recall for specifics. When grounding surfaces no verifiable source, defer per the FACT TRUST LADDER rung 5 — name the specific authority or say "Search \'[State] [resource type]\'", never substitute plausible-sounding training details. The validator strips any specific claim that didn\'t trace to grounding or pre-fetched context.';
      }
      ...
      async function callGemini(msgs) {
        ...
        // ============================================================
        // SERVER-ENFORCED LOOKUP FALL-THROUGH (punchlist2 item 1)
        // The invariant "lookup empty → search → cite → advise" no longer
        // depends on the model choosing to call request_web_search. If the most
        // recent user turn carried a lookup_* tool_result with no verified data,
        // run grounding HERE with a fact-class query and splice the results into
        // the turn the model sees (tagged VERIFIED LOOKUP / SEARCH RESULTS), so
        // Sam synthesizes with citations instead of bare-deferring. Shares the
        // 2-search/turn grounding budget with the request_web_search escape
        // hatch. Prompt-level fall-through language stays as reinforcement.
        // ============================================================
        let _fallthroughGroundingUsed = 0;
        {
          const LOOKUP_NAMES = new Set(['lookup_finance_reports', 'lookup_compliance_deadlines', 'lookup_donation_limits', 'lookup_jurisdiction']);
          const useById = new Map();
          for (const m of maskedMsgs) {
            if (m && m.role === 'assistant' && Array.isArray(m.content)) {
              for (const b of m.content) if (b && b.type === 'tool_use' && b.id) useById.set(b.id, { name: b.name, input: b.input || {} });
            }
          }
          // Consider only the most recent tool-result-bearing user turn.
          const emptyLookups = [];
          for (let i = maskedMsgs.length - 1; i >= 0; i--) {
            const m = maskedMsgs[i];
            if (!m || m.role !== 'user' || !Array.isArray(m.content)) continue;
            const trs = m.content.filter(b => b && b.type === 'tool_result');
            if (trs.length === 0) continue;
            for (const b of trs) {
              const use = b.tool_use_id ? useById.get(b.tool_use_id) : null;
              if (use && LOOKUP_NAMES.has(use.name) && lookupResultLacksData(b.content) && !emptyLookups.some(e => e.name === use.name)) {
                emptyLookups.push(use);
              }
            }
            break; // stop at the first (most recent) turn that carried tool results
          }
          const _blocks = [];
          for (const use of emptyLookups) {
            if (_fallthroughGroundingUsed >= 2) break; // shared 2-search/turn cap
            const _q = buildLookupFallthroughQuery(use.name, use.input);
            _fallthroughGroundingUsed++;
            const _sub = await runGroundingSubturn(_q);
            const _srcs = (_sub.sources && _sub.sources.length)
              ? '\nSources: ' + _sub.sources.map(s => (s && (s.uri || s.title)) ? `[${s.title || 'source'}](${s.uri || ''})` : '').filter(Boolean).join(', ')
              : '';
            _blocks.push(
              `[VERIFIED LOOKUP — ${use.name}: no verified data on file for this race.]\n` +
              `[SEARCH RESULTS (live web — synthesize from these, cite the sources inline, and add a one-line note to confirm with the named authority before acting): ${_sub.excerpt || 'no results found'}${_srcs}]`
            );
          }
          if (_blocks.length && geminiContents.length) {
            const _last = geminiContents[geminiContents.length - 1];
            const _augment = { text: '\n\n' + _blocks.join('\n\n') };
            if (_last && _last.role === 'user' && Array.isArray(_last.parts)) _last.parts.push(_augment);
            else geminiContents.push({ role: 'user', parts: [_augment] });
          }
        }
        ...
        // is a safety net. Budget starts at the count already spent by the
        // server-enforced lookup fall-through above.
        let groundingUsed = _fallthroughGroundingUsed;
